...
const CONFIG = {
  ...
  // 拉取数量：拉到 API 上限，让大图库尽可能多展示
  // （代理 functions/api/images.js 用 clampInt 强制 ≤100）
  perPage: 100,

  /* ------------------------------------------------------------
   * 七、角色图集配置（图集页使用）
   * ------------------------------------------------------------
   * 角色允许表（allowlist）。AI 工具 tools/ai/build-character-tags.mjs
   * 只会从这张表里识别角色；不在表里的标签会被前端视为"未分类"。
   * 初始为空；填几个常用角色即可（建议 5~30 项，过多会拖慢 VLM 决策）。
   * 格式：{ name: '中文名', aliases: ['英文/别称', ...] }
   * ---------------------------------------------------------- */
  characters: [
    // { name: '德克萨斯', aliases: ['Texas', '德狗', 'texas'] },
    // { name: '星熊勇仪', aliases: ['Hoshiguma', '星熊'] },
  ],
   ...
   * 六、相册列表（图集页使用）
   * ------------------------------------------------------------
   * @deprecated 自角色图集上线后,图集页改为按 CONFIG.characters +
   * js/character-tags.js 聚合角色相册。本字段仅在角色数据完全为空时
   * 作为兜底显示（兼容旧版 demo）。新增内容请优先维护 characters。
   *
   * 结构：每项 = 一个相册
